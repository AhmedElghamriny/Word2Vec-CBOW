...
class CBOWModel:
    def __init__(self, vocab_size: int, embedding_dim: int, context_size: int) -> None:
        ...
        #Initialize layers with correct dimensions whenever applicable
    ...
    def forward(self, context_indices: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: Predicted probabilities for the target word
                       (shape: (batch_size, vocab_size)).
        """
        # Lookup embeddings for context words from the embedding layer.
        embeddings = self.embedding_layer.forward(context_indices)  # Shape: (batch_size, 2 * context_size, embedding_dim)

        # Concatenate embeddings for context words. You need to go from shape (batch_size, 2 * context_size, embedding_dim)
        # to shape (batch_size, 2 * context_size * embedding_dim).
        batch_size = embeddings.shape[0]
        embeddings_reshaped = embeddings.reshape(batch_size, embeddings.shape[1] * embeddings.shape[2])

        # Feed the `embeddings_reshaped` through the linear layer.
        logits = self.linear_layer(embeddings_reshaped)  # Shape: (batch_size, vocab_size)

        # Convert logits to probabilities using the softmax layer.
    ...
    def backward(self, grad_output: np.ndarray) -> None:
        ...
        Args:
            grad_output (np.ndarray): Gradient of the loss with respect to the output probabilities
                                     (shape: (batch_size, vocab_size)).
        """
        # Backpropagate through softmax layer using the backward() method.
        grad_logits = self.softmax_layer.backward(grad_output)  # Shape: (batch_size, vocab_size)

        # Backpropagate through the linear layer using the backward() method.
    ...
    def compute_loss(self, context_indices: np.ndarray, target_indices: np.ndarray) -> float:
        ...
        """
        # Call the class's forward method with the context indices to get the predicted probabilities.
        probs = self.forward(context_indices) 

        # Compute loss by calling the loss function forward method.
        loss = self.loss_fn.forward(probs, target_indices)
        return loss
